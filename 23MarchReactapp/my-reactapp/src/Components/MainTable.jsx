...
/////////////////////////////////////////////////////////////
// Temoporary logic for onclick of ellipsis icon menu will show like show record history
const [showMenu,setShowMenu]=useState(false)
const toggleShowMenu=(id)=>{
    setShowMenu((prev) => (prev === id ? null : id)); // toggle by id
}
        ...
        <tbody>
          {projectsREDUX.map((project) => (
            <React.Fragment key={project.id}>
              {/* Project Row */}
              <tr>
                <td className="text-center d-flex justify-content-center align-items-center gap-2" style={{position:"relative"}}>
                  <FaChevronDown
                  ...
                  <FaEllipsisV
                    style={{ cursor: "pointer" }}
                    onClick={() => toggleShowMenu(project.id)}
                  />
                  {showMenu ==project.id && (
                    <div style={{width:"160px",position:"absolute",backgroundColor:"white",top:"30px",right:"0px"}}>
                      <div>Show Record History</div>
                      <div>Changed Details</div>
                      ...
                      <div className="card-header d-flex justify-content-between">
                        <div>
                          <button
                            ...
                            onClick={() =>
                              handleTabChange(project.id, "user_stories")
                            }
                          >
                            User Stories
                          </button>
                          <button
                            className={`btn ${
                              project.tabs.active_tab === "releases"
                                ? "btn-primary"
                                : "btn-outline-primary"
                            }`}
                            onClick={() =>
                              handleTabChange(project.id, "releases")
                            }
                          >
                            Releases
                          </button>
